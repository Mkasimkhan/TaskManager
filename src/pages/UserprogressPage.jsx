...
//         ? remarks
//           .map((r) => `${r.text}${r.time ? ` (${dayjs(r.time).format("DD MMM, hh:mm A")})` : ""}`)
//           .join(" | ")
//         : ""
//   },
// ];
...
//                       <th
//                         key={col.key}
//                         className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider text-indigo-600"
//                       >
//                         {col.header}
//                       </th>
//                     ))}
    ...
    render: (remarks) =>
      Array.isArray(remarks) && remarks.length > 0
        ? remarks
          .map(
            (r) =>
              `[${r.time ? dayjs(r.time).format("DD MMM, h:mm A") : "N/A"}] ${r.text}`
          )
          .join(" | ")
        : "—"
  }
